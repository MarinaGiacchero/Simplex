...
function StateController() {
    ...
    var PersistenceRegion  = require('../persistence/region.js');
    var persistenceRegion  = new PersistenceRegion();
    ...
    this.getQttByInitials = function (req, res) {
        persistence.getQttByInitials(req.params.initials)
        .then((qtt) => {
            res.json({ qtde: qtt });
    ...
    this.isNameInUse = function (name, res) {
        ...
        .then((isNameInUse) => {
            res.json({ nomeEmUso: isNameInUse ? "Sim" : "Não" });
            console.log("Nome do estado está em uso? " + isNameInUse ? "Sim" : "Não");
        });
    };
    ...
    this.isInitialsInUse = function (initials, res) {
        ...
        .then((isInitialsInUse) => {
            res.json({ siglaEmUso: isInitialsInUse ? "Sim" : "Não" });
            console.log("Sigla do estado está em uso? " + isInitialsInUse ? "Sim" : "Não");
        });
    };
...
/*
            new Promise((resolve, reject)=>{
                console.log("aqui " + req.body.regionId + " " + req.body.initials);
                persistenceRegion.getById(req.body.regionId, res)
                .then(
                    // resolve
                    (data) => {
                        if (data) {
                            error  = {  location: 'body', 
                                        param:    'regionId', 
                                        msg:      'Região não encontrada', 
                                        value:     req.body.regionId
                                    };
                    ...
                    }, // fim do resolve getById para região
                    // reject
                    function ( erro ) {
                        reject(erro);
                    }// fim do reject getById para região
                ); // fim do then getById para região
            })
*/
          ]).then(result=>{
