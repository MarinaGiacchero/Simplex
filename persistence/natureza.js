...
var dataBase   = new DataBase();
 ...
 function NaturezaPersistence() {

    this.getPersistence = 
    function () {    
          return new Promise(function (resolve, reject) {
              if   (globals.dataBaseType == 1) {
                    var Persistence = require('./relational/natureza.js');
                    var persistence = new Persistence();
                    resolve(persistence);
                  }
              else if   (globals.dataBaseType == 2) {
                  /*firebase ainda não possui o model personagem*/
                          var Persistence = require('./firebase/natureza.js');          
                          var persistence = new Persistence();
                          resolve(persistence);
                  }
                   else {
...
module.exports = NaturezaPersistence;
